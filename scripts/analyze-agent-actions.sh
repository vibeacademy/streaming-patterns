...
check_pr_actions() {
  ...
  gh pr list --state all --limit 100 --json number,title,createdAt,author,mergedAt,mergedBy --search "created:>=$SINCE_DATE" > "$TMP_DIR/prs.json"

  # Parse PR data
  if [ -s "$TMP_DIR/prs.json" ]; then
    # Count PRs by bot authors
    jq -r '.[] | select(.author.login | contains("bot") or contains("github-actions")) | "\(.createdAt)|\(.author.login)|pr_created|{\"pr\":\(.number),\"title\":\"\(.title)\"}"' "$TMP_DIR/prs.json" | while IFS='|' read -r timestamp agent action context; do
      log_action "$timestamp" "$agent" "$action" "$context" "false"
    done

    # Check for bot merges (restricted action)
    jq -r '.[] | select(.mergedBy != null) | select(.mergedBy.login | contains("bot") or contains("github-actions")) | "\(.mergedAt)|\(.mergedBy.login)|merge_pull_request|{\"pr\":\(.number),\"title\":\"\(.title)\"}"' "$TMP_DIR/prs.json" | while IFS='|' read -r timestamp agent action context; do
...
check_workflow_actions() {
  ...
  gh run list --limit 100 --json workflowName,createdAt,conclusion,name --created ">=$SINCE_DATE" > "$TMP_DIR/workflows.json"

  if [ -s "$TMP_DIR/workflows.json" ]; then
    # Check for production deployments
    jq -r '.[] | select(.workflowName | contains("deploy-production") or contains("Deploy to Production")) | "\(.createdAt)|github-actions|deploy_production|{\"workflow\":\"\(.workflowName)\",\"conclusion\":\"\(.conclusion)\"}"' "$TMP_DIR/workflows.json" | while IFS='|' read -r timestamp agent action context; do
      # Production deploys should require manual approval
      # Flag as restricted if they appear to be automatic
      restricted="false"
      if echo "$context" | grep -q "\"conclusion\":\"success\""; then
        # This is expected - human approved and deployed
        restricted="false"
      fi
      log_action "$timestamp" "$agent" "$action" "$context" "$restricted"
      DEPLOY_PROD_ATTEMPTS=$((DEPLOY_PROD_ATTEMPTS + 1))
    done

    # Check for staging deployments (not restricted)
    jq -r '.[] | select(.workflowName | contains("deploy-staging") or contains("Deploy to Staging")) | "\(.createdAt)|github-actions|deploy_staging|{\"workflow\":\"\(.workflowName)\",\"conclusion\":\"\(.conclusion)\"}"' "$TMP_DIR/workflows.json" | while IFS='|' read -r timestamp agent action context; do
      log_action "$timestamp" "$agent" "$action" "$context" "false"
    done
  fi
}
...
check_issue_actions() {
  ...
  # Note: This requires the gh CLI to support timeline events
  # For now, we'll skip this as it requires more complex API calls
...
EOF
    ...
    echo "─────────────────────────────────────────────────────────────────"
    if [ -f "$TMP_DIR/actions.jsonl" ]; then
      jq -r '"\(.timestamp) | \(.agent) | \(.action)"' "$TMP_DIR/actions.jsonl" | head -n 20 || true
    fi
    echo ""
