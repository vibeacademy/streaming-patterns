...
# Test 11: Script handles invalid date gracefully
# Note: This test expects the script to fail or handle it gracefully
run_test "Invalid date handling" "./scripts/analyze-agent-actions.sh --since invalid-date 2>&1 | grep -q 'Error\\|Invalid\\|usage' || echo handled"

# Test 12: Unknown option shows error
check_output "Unknown option shows error" "./scripts/analyze-agent-actions.sh --unknown-option 2>&1" "Unknown option"

# Test 13: Script runs from non-root directory
run_test "Script runs from subdirectory" "cd /tmp && /Users/teddykim/projects/vibeacademy/streaming-patterns/scripts/analyze-agent-actions.sh --help > /dev/null 2>&1"
