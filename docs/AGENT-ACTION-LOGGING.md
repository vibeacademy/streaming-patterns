...
This document describes the agent action logging and audit trail system implemented to track agent behavior, detect restricted action attempts, and maintain compliance with the Agent Workflow Hardening initiative.
...
| `agent` | string | Agent identity (e.g., "github-actions", "pr-reviewer") |
...
1. **Real-Time Logging**
   - MCP server middleware integration
   - Live action tracking during agent sessions
   - WebSocket-based log streaming

2. **Enhanced Context**
   - Capture full MCP tool parameters
   - Track agent reasoning/justification
   - Record instruction context
...
4. **Project Board Tracking**
   - GitHub GraphQL API integration
   - Track issue column movements
   - Detect automated "Move to Done" actions

5. **Audit Report Dashboard**
...
- Created `analyze-agent-actions.sh` script
- Implemented weekly audit workflow
- Added JSON and human-readable report formats
- Integrated restricted action detection
- Documented log format and usage

---

*This documentation is part of the Agent Workflow Hardening initiative (Issue #151).*
